...
//What is an Action ? It is a placeholder for a Method with no Params
template<typename T> std::function<void(T)> customAction;

//what is a Func? It is a placeholder for a Method that returns a Value
template <typename T, typename U> std::function<U(T, T)> customFunc;
...
void LambdaExamples(){
    ...
    customAction<std::string> = PrintMessage;
    customAction<std::string>("Hello World!");

    customFunc<int,int> = Add;
    qInfo() << "Func result: " << customFunc<int, int>(2,3);
}
