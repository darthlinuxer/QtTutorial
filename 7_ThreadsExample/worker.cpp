...
{
    QThread::currentThread()->setObjectName("Worker"+QString::number(workerId++));
...
void Worker::start()
{
    ...
    qInfo() << this << "Event Loop finished on " << QThread::currentThread();
...
void Worker::timeout()
{
    qInfo() << this << seconds++ << ":Timeout on " << QThread::currentThread();
}
